...
from convlstm_new.model import ConvLSTMModel, PhysicsEnhancedConvLSTM
...
from common_utils.dataloader import create_datasets_full, create_datasets_lite, create_dataloaders
...
class Trainer:
    ...
    def train_loop(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            # Logic for curriculum learning or warm-up (Parity with AFNO script)
            ...
            for batch_idx, (inp, tar, _) in enumerate(pbar):
                ...
                with torch.amp.autocast(device_type=self.device_type, dtype=self.mixed_prec_dtype):
                    # print(inp.shape)
                    outputs = self.model(inp)  # Forward pass
                    
                    # --- BACKWARD COMPATIBILITY BLOCK ---
                    if isinstance(outputs, tuple):
                        # Enhanced Model with Deep Supervision
                        final_pred, inter_pred = outputs
                        
                        # Slice both to match target duration
                        final_pred = final_pred[:, :self.config.num_pred_frames, ...]
                        inter_pred = inter_pred[:, :self.config.num_pred_frames, ...]
                        
                        # Calculate Main Loss
                        loss_final, components_final = self.loss_fn(final_pred, tar)
                        
                        # Calculate Auxiliary/Intermediate Loss
                        loss_inter, _ = self.loss_fn(inter_pred, tar)
                        
                        # Combine: Total = Final + 0.4 * Intermediate
                        loss = loss_final + 0.4 * loss_inter
                        
                        # Use final components for logging
                        loss_components = components_final
                    else:
                        # Standard Model (Backward Compatible)
                        pred = outputs
                        pred = pred[:, :self.config.num_pred_frames, ...]
                        loss, loss_components = self.loss_fn(pred, tar)
                    # ------------------------------------

                    loss /= eff_accum_steps  # Normalize loss for gradient accum
        ...
        with torch.no_grad():  # Disable gradient computation for validation
            for inp, tar, _ in self.val_loader:
                ...
                with torch.amp.autocast(device_type=self.device_type, dtype=self.mixed_prec_dtype):
                    ...
                    # --- BACKWARD COMPATIBILITY BLOCK ---
                    if isinstance(outputs, tuple):
                        pred = outputs[0] # Discard intermediate output for validation
                    else:
                        pred = outputs
                    # ------------------------------------
...
def run_seq2seq_test(model, test_loss_fn, test_loader, config, temp_run_dir):
    ...
    # Force physics weights to 1.0 for standardized testing reporting
    ...
    # Dense evaluation stride (match AFNO script)
    stride = 1 
    ...
    with torch.no_grad():
        ...
        for sample_idx, (full_sequence_inputs, full_sequence_targets) in enumerate(test_loader):
            ...
            for t in range(0, loop_end, stride):
                ...
                # 3. Predict full sequence
                pred_seq = model(input_chunk) 

                if isinstance(pred_seq, tuple):
                    pred_seq = pred_seq[0] # Take only final prediction
                ...
                # Extra check for burning MSE (explicit)
            ...
            if sample_idx in samples_to_visualize:
                ...
                # Stitch chunks for visualization
                predictions_np_viz = torch.cat(predictions_for_viz, dim=0).squeeze(1).cpu().numpy()
                ground_truth_np_viz = torch.cat(ground_truths_for_viz, dim=0).squeeze(1).cpu().numpy()
...
def main(args_config):
    ...
    with tempfile.TemporaryDirectory() as temp_run_dir:
        ...
        train_loader, val_loader, test_loader = create_dataloaders(train_dataset, val_dataset, test_dataset, config, test_batch_size=1)
        config.in_channels = next(iter(train_loader))[0].shape[2] # (B, T, C, H, W) -> C is at index 2


        model =  PhysicsEnhancedConvLSTM(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="/home/pgmlvol/tcaglar/pgml/convlstm_new/config_lite_data.yaml", help="Path to the config file")
