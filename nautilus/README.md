...
Use `kubens` to easily switch between namespaces. For example, see [here](https://github.com/ahmetb/kubectx?tab=readme-ov-file#examples)
```
kubens <namespace>
```

To install `kubens` in macOS using homebrew: `brew install kubens`
More detailed installation instructions can be found [here](https://github.com/ahmetb/kubectx?tab=readme-ov-file#installation)

Use kubectl `cp` command to copy data from local to remote pod / pvc volume
```
kubectl cp /path/to/local/data <pod-name>:/path/to/pvc/mount/point -c <container-name>
```

Similarly, to download data from remote pod / pvc volume to local, interchange source and destination:
```
kubectl cp <pod-name>:/path/to/pvc/mount/point /path/to/local/destination -c <container-name>
```

If you are opening the pod container inside VS code, install extensions: Jupyter, Python